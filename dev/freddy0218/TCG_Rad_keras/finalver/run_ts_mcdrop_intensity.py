...
def prepare_tensors(filepath=None,splitnum=None,explag=23):
    ...
    # Maria
    ############################################################################################################################################
    #Xtrain = read_and_proc.depickle(glob.glob(filepath+'X/'+str(splitnum)+'/*corr*')[0])['uvwthhdialwsw_inten']['train']
    #Xvalid = read_and_proc.depickle(glob.glob(filepath+'X/'+str(splitnum)+'/*corr*')[0])['uvwthhdialwsw_inten']['valid']
    #Xtest = read_and_proc.depickle(glob.glob(filepath+'X/'+str(splitnum)+'/*corr*')[0])['uvwthhdialwsw_inten']['test']
    ...
    print(np.asarray(X_totrain).shape)
...
if filepath=='./maria_store/':
    nummem = [0,50,26,50,50,50,10,10,1]
elif filepath=='./haiyan_store/':
    nummem = [0,50,38,50,8,50,20,20,1]

def objective(trial):
    models,losses = [],[]
    #droprate = trial.suggest_float("droprate",0.05,0.45)
    model = ts_models.OptimMLR_lwswinten_3D_ts_dropout(droprate,nummem)
...
if droprate==0.0:
    ...
    read_and_proc.save_to_pickle(filepath+'dropout_inten_80/'+str(splitnum)+'/'+str(droprate)+'/bestparams.pkt',study,'PICKLE')
else:
    read_and_proc.save_to_pickle(filepath+'dropout_inten_80/'+str(splitnum)+'/'+str(droprate)+'/bestparams.pkt',study,'PICKLE')
#if droprate==0.0:
#    droprate = int(droprate)
#    study = read_and_proc.depickle(filepath+'dropout_inten/'+str(splitnum)+'/'+str(droprate)+'/bestparams.pkt')
#else:
#    study = read_and_proc.depickle(filepath+'dropout_inten/'+str(splitnum)+'/'+str(droprate)+'/bestparams.pkt')
#study = read_and_proc.depickle('./store/dropout_inten/'+str(expname)+'/'+str(droprate)+'/'+str(splitnum)+'/bestparams.pkt')
...
for i in range(len(times)):
    ...
    for optimizer in optimizers:
        ...
        early_stopper = ts_models.EarlyStopping(patience=400, verbose=False, delta=1e-5, path='checkpoint.pt', trace_func=print)#EarlyStopper(patience=8, min_delta=1e-3)
    ...
    torch.save(models, filepath+'dropout_inten_80/'+str(splitnum)+'/'+str(droprate)+'/modelstest'+str(splitnum)+'_lwswts_1115_'+str(times[i])+'.pt')
    read_and_proc.save_to_pickle(filepath+'dropout_inten_80/'+str(splitnum)+'/'+str(droprate)+'/lossestest'+str(splitnum)+'_lwswts_1115_'+str(times[i])+'.pkt',losses,'PICKLE')
