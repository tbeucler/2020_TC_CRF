...
def objective(trial):
    ...
    model = ts_models.OptimMLR_lwsw_3D_ts(droprate)
...
read_and_proc.save_to_pickle('../tmp/deterministic/'+str(expname)+'2/'+str(splitnum)+'/bestparams.pkt',study,'PICKLE')
...
for i in range(len(times)):
    ...
    if expname=='lwswv':
        ...
        model = ts_models.OptimMLR_lwsw_3D_ts(study.best_params['droprate'])
    elif expname=='lwswu':
    ...
    optimizers = [torch.optim.Adam(model.parameters(), lr=study.best_params['lr'])]#, optim.AdaBound(model.parameters(),lr=1e-7)] 1e-6
    ...
    for optimizer in optimizers:
        ...
        models.append(model)
        losses.append(loss)
    torch.save(models, '../tmp/deterministic/'+str(expname)+'2/'+str(splitnum)+'/models'+str(splitnum)+'_'+str(expname)+'3dnonln_1115_'+str(times[i])+'.pt')
    read_and_proc.save_to_pickle('../tmp/deterministic/'+str(expname)+'2/'+str(splitnum)+'/losses'+str(splitnum)+'_'+str(expname)+'3dnonln_1115_'+str(times[i])+'.pkt',losses,'PICKLE')
