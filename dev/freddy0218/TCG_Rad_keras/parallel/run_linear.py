...
for splitnum in range(20,32):#range(33):
    ...
    for i in range(len(times)):
        models,losses = [],[]
        if expname=='lwswhdia':
            model = linear_models.OptimMLR_all_3D_simple_lwswhdia()
        elif expname=='lwswu':
            model = linear_models.OptimMLR_all_3D_simple_lwswu()
        elif expname=='lwswv':
            model = linear_models.OptimMLR_all_3D_simple_lwswv()
        elif expname=='lwsww':
            model = linear_models.OptimMLR_all_3D_simple_lwsww()
        elif expname=='lwswth':
            model = linear_models.OptimMLR_all_3D_simple_lwswth()
        elif expname=='lwswthu':
            model = linear_models.OptimMLR_all_3D_simple_lwswthu()
        elif expname=='lwswthv':
            model = linear_models.OptimMLR_all_3D_simple_lwswthv()
        elif expname=='lwswthw':
            model = linear_models.OptimMLR_all_3D_simple_lwswthw()
        elif expname=='lwswthhdia':
            model = linear_models.OptimMLR_all_3D_simple_lwswthhdia()
        elif expname=='lwswthhdiau':
            model = linear_models.OptimMLR_all_3D_simple_lwswthhdiau()
        elif expname=='lwswthhdiav':
            model = linear_models.OptimMLR_all_3D_simple_lwswthhdiav()
        elif expname=='lwswthhdiaw':
            model = linear_models.OptimMLR_all_3D_simple_lwswthhdiaw()
        print(expname,type(expname))
        print(model)
        ...
        for optimizer in optimizers:
            ...
            early_stopper = linear_models.EarlyStopping(patience=50, verbose=False, delta=1e-7, path='checkpoint.pt', trace_func=print)#EarlyStopper(patience=8, min_delta=1e-3)
            #variance_store = [varu,varv,varw,varth]
            #variance_store = [varu,varv,varth]
            model,loss = linear_models.train_model(model=model,optimizer=optimizer,scheduler=[scheduler,scheduler2],numepochs=num_epochs,early_stopper=early_stopper,variance_store=None,\
                                     lossfunc=loss,train_loader=train_loader,val_loader=val_loader,test_loader=test_loader)
            models.append(model)
            losses.append(loss)
        torch.save(models, '../tmp/torch_try/'+str(expname)+'/0/'+'models'+str(splitnum)+'_'+str(expname)+'3dnonln_1115_'+str(times[i])+'.pt')
        read_and_proc.save_to_pickle('../tmp/torch_try/'+str(expname)+'/0/'+'losses'+str(splitnum)+'_'+str(expname)+'3dnonln_1115_'+str(times[i])+'.pkt',losses,'PICKLE')
